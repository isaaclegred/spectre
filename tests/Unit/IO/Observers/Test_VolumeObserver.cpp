...
SPECTRE_TEST_CASE("Unit.IO.Observers.VolumeObserver", "[Unit][Observers]") {
  ...
  std::vector<std::string> grid_names = volume_file.get_grid_names(temporal_id);
  ...
  for (const auto& element_id : element_ids) {
    ...
    pairs.push_back(std::make_tuple(element_id, std::distance(start, place)));
  }
  ...
  std::sort(pairs.begin(), pairs.end(),
            [](std::tuple<ElementId<2>, size_t> pair_1,
               std::tuple<ElementId<2>, size_t> pair_2) {
  ...
  for (auto& tensor_name : volume_file.list_tensor_components(temporal_id)) {
  ...
  std::vector<std::vector<size_t>> read_extents =
  ...
  for (size_t i = 0; i < sorted_element_ids.size(); i++) {
    auto element_id = sorted_element_ids[i];
    ...
    size_t stride = [&volume_data_fakes]() {
      size_t local_points = 1;
      for (auto& extent : std::get<0>(volume_data_fakes)) {
        local_points *= extent;
      }
      return local_points;
    }();
    ...
    for (const auto& tensor_component : std::get<1>(volume_data_fakes)) {
      CHECK(tensor_component.data ==
            DataVector(
                &read_tensor_data[tensor_component.name][0] + points_processed,
                stride));
    }
