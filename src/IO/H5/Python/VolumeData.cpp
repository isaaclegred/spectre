      ...
      .def("get_grid_names",
           +[](h5::VolumeData& V, size_t observation_id) {
             return std_vector_to_py_list<std::string>(
                 V.get_grid_names(observation_id));
           })
      .def("list_tensor_components",
           +[](h5::VolumeData& V, size_t observation_id) {
             return std_vector_to_py_list<std::string>(
                 V.list_tensor_components(observation_id));
           })
      .def("get_tensor_component",
           +[](h5::VolumeData& V, size_t observation_id,
               const std::string& tensor_component) {
             return V.get_tensor_component(observation_id, tensor_component);
           })
      .def("get_extents", +[](h5::VolumeData& V, size_t observation_id) {
        bp::list total_extents;
        for (const auto& extents : V.get_extents(observation_id)) {
