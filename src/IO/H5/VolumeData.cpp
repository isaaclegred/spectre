...
void append_element_quadratures(
    const gsl::not_null<std::vector<int>*> quadratures,
    const ElementVolumeData& element, const size_t dim) {
  for (size_t axis = 0; axis < dim; axis++) {
    quadratures->push_back(static_cast<int>(element.quadrature[axis]));
  }
}

void append_element_bases(const gsl::not_null<std::vector<int>*> bases,
                          const ElementVolumeData& element, const size_t dim) {
  for (size_t axis = 0; axis < dim; axis++) {
    bases->push_back(static_cast<int>(element.basis[axis]));
  }
}
...
void VolumeData::write_volume_data(
  ...
  const std::vector<std::string> component_names(
      ...
      boost::make_transform_iterator(elements.front().tensor_components.end(),
                                     get_component_name));
  // The dimension of the grid is the number of extents per elements. I.e., if
  // the extents are [8,5,7] for any element, the dimension of the grid is 3.
  // Only written once per VolumeData file.
  ...
  h5::write_dictionary("Quadrature dictionary", h5::IO_quadrature_names,
                       observation_group);
  h5::write_data(observation_group.id(), quadratures, {quadratures.size()},
                 "quadratures");
  // Write the coded basis, along with the dictionary
  h5::write_dictionary("Basis dictionary", h5::IO_basis_names,
                       observation_group);
...
std::vector<std::string> VolumeData::list_tensor_components(
  ...
  auto remove_data_name = [&tensor_components](const std::string& data_name) {
    return static_cast<void>(std::remove(tensor_components.begin(),
                                         tensor_components.end(), data_name));
  };
...
std::vector<std::vector<std::string>> VolumeData::get_bases(
  ...
  auto all_bases = h5::decode_with_dictionary_name(
...
std::vector<std::vector<std::string>> VolumeData::get_quadratures(
  ...
  auto all_quadratures = h5::decode_with_dictionary_name(
